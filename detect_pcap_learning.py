...
def run(
    ...
    if is_pcap:
        ...
        with closing(client.Scans(pcap_file)) as scans:
            ...
            vid_writerHumanSegmentation = cv2.VideoWriter(
                save_pathHumanSegmentation,
                    ...
                    if len(det):
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            if save_img or save_crop or view_img:  # Add bbox to image
                                ...
                                # plot 3d points of detected human
                                colors_detected = ["y", "g", "b", "k"]
                                color_choice = colors_detected[n_detected % len(colors_detected)]
                                ax.scatter(
                                    multiple_xyz_list_reduced[:, 0],
                                    multiple_xyz_list_reduced[:, 1],
                                    multiple_xyz_list_reduced[:, 2],
                                    c=color_choice,
                                    marker="o",
                                    alpha=0.3,
                                )
                                ...
                                if displacement_computed:
                                    ax.arrow3D(
                                        ...
                                        displacement[0] * 30,
                                        displacement[1] * 30,
                                        0,  # prediction on next 30 frames
                                        mutation_scale=10,
                                        ec="red",
                                        fc="red",
                                        alpha=1,
                                ...
                                ax.scatter(
                                    closestPointToCenterOfMass[0],
                                    closestPointToCenterOfMass[1],
                                    closestPointToCenterOfMass[2],
                                    c="g",
                                    marker="o",
                                )
...
def parse_opt():
    ...
    parser.add_argument(
        "--social-distance",
        action="store_true",
        help="calculate distance between two people",
    )

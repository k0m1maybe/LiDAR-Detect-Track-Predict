...
def run(
    ...
    if is_pcap:
        ...
        with closing(client.Scans(pcap_file)) as scans:
            ...
            vid_writerObjectSegmentation = cv2.VideoWriter(
                save_pathObjectSegmentation,
            ...
            for scan in scans:
                ...
                detected_objects = np.zeros(range_val.shape, dtype=np.uint8)
                xyz_detected = []
                poi_list = []
                ...
                for i in range(len(xyz_detected_clustered)):
                    ...
                    ax.scatter(
                        xyz_detected_clustered[i][:, 0],
                        xyz_detected_clustered[i][:, 1],
                        xyz_detected_clustered[i][:, 2],
                        s=2,
                    )
                ...
                for i in range(n_detected):
                    ...
                    # plot the centerOfMass and the moving direction
                    ax.scatter(
                        centerOfMass[0],
                        centerOfMass[1],
                        centerOfMass[2],
                ...
                # presenting by coloring objects
